...
<div class="wrap">
 ...
 <h2><?php _e( 'TCD Theme Options', 'tcd-issue' ); ?></h2> 
 ...
 <div class="updated fade">
  <p><strong><?php _e( 'Updated', 'tcd-issue' ); ?></strong></p>
 </div>
 <?php endif; ?>
 ...
 <div id="my_theme_option" class="cf">
  ...
  <div id="my_theme_right">
   ...
   <div id="saving_data" style="display:none;"><p><?php _e('Now saving...', 'tcd-issue'); ?></p></div>
  </div>
...
require get_template_directory() . '/admin/inc/staff.php';
...
function theme_options_tools_notices() {
	...
	if ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'error' === $tools_result[1] ) {
		...
		if ( isset( $tools_result[2] ) ) {
			...
			switch( $int_import_error ) {
				...
				case 8:
					...
					break;
				case 11:
                  $message = $int_import_error . ' : ' . __( 'File has not been uploaded.', 'tcd-issue' );
					break;
				case 12:
                  $message = $int_import_error . ' : ' . __( 'There is no file', 'tcd-issue' );
					break;
				case 13:
                  $message = $int_import_error . ' : ' . __( 'The file extension is not .json', 'tcd-issue' );
					break;
				case 14:
                  $message = $int_import_error . ' : ' . __( 'Failed to read the file', 'tcd-issue' );
					break;
				case 15:
                    $message = $int_import_error . ' : ' . __( 'Json decoding failed', 'tcd-issue' );
					break;
				default :
		...
		echo '<div class="update-message notice notice-error notice-alt"><p><strong>' . sprintf( __( 'Import error. %s', 'tcd-issue' ), $message ) . '</strong></p></div>';

	// インポート成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'import' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings imported', 'tcd-issue' ) . '</strong></p></div>';

	// リセット成功メッセージ
	} elseif ( isset( $tools_result[0], $tools_result[1] ) && 'reset' === $tools_result[0] && 'success' === $tools_result[1] ) {
        echo '<div class="updated"><p><strong>' . __( 'Settings reset', 'tcd-issue' ) . '</strong></p></div>';
	}
}
...
/**
 * issue オプションTools デフォルト画像設定フィルター
 */
function issue_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		...
		array(
			'media_filename' => 'image_1740x1320.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1740x1320.gif',
			'theme_option_keys' => array("index_slider_image","['contents_builder'][2]['image1']")
		),
		array(
			'media_filename' => 'image_banner.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_banner.gif',
			'theme_option_keys' => array("['contents_builder'][1]['image']")
		),
		array(
			'media_filename' => 'image_400x400.png',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_400x400.png',
			'theme_option_keys' => array("['contents_builder'][2]['image22']")
		),
		array(
			'media_filename' => 'image_870x340.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_870x340.gif',
...
/**
 * issue オプションTools サンプルカテゴリー
 */
function issue_theme_options_tools_set_sample_categories( $dp_options ) {
  ...
  // スタッフカテゴリー
	for ( $i = 1; $i <= 3; $i++ ) {
		$sample_categories[] = array(
			'name' => __( 'Category', 'tcd-issue' ) . $i,
			'slug' => 'staff_category'.$i,
			'taxonomy' => 'staff_category',
			'description' => __( 'Description will be displayed here.', 'tcd-issue' ),
		);
	}
...
/**
 * issue オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function issue_theme_options_tools_set_sample_posts( $dp_options ) {

	// ブログ、お知らせのアイキャッチ画像
	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_715x410.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_715x410.gif'
	), true );

	// スタッフのアイキャッチ画像
	$staff_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_725x1200.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_725x1200.gif'
	), true );

	// インタビューのアイキャッチ画像
	$interview_media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'image_1740x720.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1740x720.gif'
	), true );
	...
	for ( $i = 1; $i <= 6; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample post', 'tcd-issue' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-issue' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample news', 'tcd-issue' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-issue' ),
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample staff', 'tcd-issue' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-issue' ),
			'post_name' => 'sample-staff' . $i,
			'post_status' => 'publish',
			'post_type' => 'staff',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $staff_media_id,
			'taxonomies' => array(
				'staff_category' => array('staff_category1','staff_category2','staff_category3')
			),
			'metas' => array(
				'staff_name' => __( 'Name', 'tcd-issue' ),
				'staff_job' => __( 'Job', 'tcd-issue' ),
				'staff_catch' => __( 'Catchphrase will be displayed here.', 'tcd-issue' ),
				'staff_date' => __( 'Supplemental information', 'tcd-issue' ),
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample conversation', 'tcd-issue' ) . $i,
			'post_content' => __( 'Sample text. Sample text.', 'tcd-issue' ),
			'post_name' => 'sample-interview' . $i,
			'post_status' => 'publish',
			'post_type' => 'interview',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $interview_media_id,
			'taxonomies' => array(
				'interview_category' => array('interview_category1','interview_category2','interview_category3')
			),
			'metas' => array(
				'interview_title' => __( 'Sample title', 'tcd-issue' ),
				'interview_sub_title' => __( 'Sample sub title', 'tcd-issue' ),
				'interview_last_name1' => __( 'Name', 'tcd-issue' ),
				'interview_department1' => __( 'Department', 'tcd-issue' ),
				'interview_occupation1' => __( 'Occupation', 'tcd-issue' ),
				'interview_position1' => __( 'Official position', 'tcd-issue' ),
			)
		);
	}
...
/**
 * issue オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function issue_theme_options_tools_set_sample_menus( $dp_options ) {
	...
	if ( $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach( $nav_menus as $nav_menu ) {
			if ( $nav_menu->name == __( 'Sample menu', 'tcd-issue' ) )
				return $dp_options;
		}
	}

	// サンプルメニュー作成
	$menu_id = wp_create_nav_menu( __( 'Sample menu', 'tcd-issue' ) );
...
/**
 * issue オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function issue_theme_options_tools_set_sample_widgets( $dp_options ) {
	...
	$sample_widgets = array(
		...
		array(
			...
			'params' => array(
        'post_num' => '3',
        'title1' => __('Recent post', 'tcd-issue'),
        'title2' => __('Recommend post', 'tcd-issue'),
        'content_type1' => 'post',
        'content_type2' => 'post',
        'post_order1' => 'date',
        'post_order2' => 'rand',
		...
		array(
			...
			'params' => array(
        'title' => __('Recommend post', 'tcd-issue'),
        'post_num' => '3',
        'content_type' => 'post',
 ...
 * issue テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'issue_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'issue_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'issue_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'issue_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'issue_theme_options_tools_set_sample_widgets', 10, 1 );
