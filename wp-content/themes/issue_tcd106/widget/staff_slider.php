...
class staff_slider_widget extends WP_Widget {

  function __construct() {
    $options = get_design_plus_option();
    $staff_label = $options['staff_label'] ? esc_html( $options['staff_label'] ) : __( 'Staff', 'tcd-issue' );
    parent::__construct(
      'staff_slider_widget',// ID
      sprintf(__('%s post slider (tcd ver)', 'tcd-issue'), $staff_label),
      array(
        'classname' => 'staff_slider_widget',
        'description' => sprintf(__('Display %s post slider.', 'tcd-issue'), $staff_label),
      )
    );
  }
  ...
  function widget($args, $instance) {
    ...
    if($post_type == 'all_post') {
      $args = array('post_type' => 'staff', 'posts_per_page' => $post_num, 'ignore_sticky_posts' => 1, 'orderby' => $post_order);
    } else {
      $args = array('post_type' => 'staff', 'posts_per_page' => $post_num, 'ignore_sticky_posts' => 1, 'orderby' => $post_order, 'meta_key' => $post_type, 'meta_value' => 'on');
    };

    $options = get_design_plus_option();
    $post_slider_query = new WP_Query($args);
?>
<div class="staff_slider_widget_wrap swiper">
 <div class="staff_slider swiper-wrapper">
  <?php
...
<div class="staff_slider_widget_pagination swiper-pagination"></div>
<?php
...
function register_staff_slider_widget() {
	register_widget( 'staff_slider_widget' );
}
add_action( 'widgets_init', 'register_staff_slider_widget' );
