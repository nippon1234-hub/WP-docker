...
function po_admin_print_scripts() {
	...
	if ('page' === $post_type || 'interview' === $post_type || 'staff' === $post_type) {
...
function change_post_order() {
	...
	if ( 'page' !== $post_type && 'interview' !== $post_type && 'staff' !== $post_type) return;
...
function po_pre_get_posts( $query ) {
  ...
  if ( is_admin() ) return;
  if ( (is_post_type_archive('interview') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
  if ( (is_tax('interview_category') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
  if ( (is_post_type_archive('staff') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
  if ( (is_tax('staff_category') && $query->is_main_query()) ) { 
    $query->set( 'orderby', array( 'menu_order' => 'ASC', 'date' => 'DESC' ) );
  }
}
...
function po_get_previous_post_where( $where, $in_same_term, $excluded_terms, $taxonomy, $post ) {
	if ( in_array( $post->post_type, ['staff'] ) ) {
		$where = preg_replace( "/ p.post_date < '.*?'/", " p.menu_order < {$post->menu_order}", $where );
	}
	if ( in_array( $post->post_type, ['interview'] ) ) {
		$where = preg_replace( "/ p.post_date < '.*?'/", " p.menu_order < {$post->menu_order}", $where );
	}
	return $where;
}
