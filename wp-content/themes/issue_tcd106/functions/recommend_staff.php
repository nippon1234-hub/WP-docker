...
function recommend_staff_meta_box() {
 add_meta_box(
  'wp_recommend_staff',//ID of meta box
  __('Recommend post', 'tcd-issue'),//label
  'recommend_staff',//callback function
  'staff',// post type
  'side'
 );
}
add_action('add_meta_boxes', 'recommend_staff_meta_box');

function recommend_staff(){
    ...
    echo '<input type="hidden" name="recommend_staff_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
...
add_action('save_post', 'save_recommend_staff_meta_box');
function save_recommend_staff_meta_box( $post_id ) {

  // verify nonce
  if (!isset($_POST['recommend_staff_meta_box_nonce']) || !wp_verify_nonce($_POST['recommend_staff_meta_box_nonce'], basename(__FILE__))) {
    return $post_id;
  }
